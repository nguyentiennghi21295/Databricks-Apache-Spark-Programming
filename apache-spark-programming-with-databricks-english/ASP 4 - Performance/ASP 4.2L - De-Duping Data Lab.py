...
df = (df
      ...
      .withColumn("salary",lower(col("salary")))
...
from pyspark.sql.functions import upper, concat,substring, lit

df = (df
      .withColumn("firstName",concat(upper(substring(col("firstName"),1,1)),substring(col("firstName"),2,10000)))
      .withColumn("middleName",concat(upper(substring(col("middleName"),1,1)),substring(col("middleName"),2,10000)))
      .withColumn("lastName",concat(upper(substring(col("lastName"),1,1)),substring(col("lastName"),2,10000)))
      .withColumn("ssn",concat(substring(col("ssn"),1,3),lit("-"),substring(col("ssn"),4,2),lit("-"),substring(col("ssn"),6,10000)))
)
df.show(20)
print(df.count())
...
coalesce_df.write.format("delta").mode("overwrite").save(delta_dest_dir)
