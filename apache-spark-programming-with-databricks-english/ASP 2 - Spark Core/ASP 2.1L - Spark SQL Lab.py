...
spark.table("events").printSchema()
...
spark.table("events").limit(5).display()

# COMMAND ----------

(spark.table("events").where("device = 'macOS'").display())

# COMMAND ----------

(spark.table("events").where("device = 'macOS'").limit(5).display())

# COMMAND ----------

df_1 = spark.sql("SELECT * FROM events WHERE device = 'macOS' ORDER BY event_timestamp ASC")
df_1.display()
...
# MAGIC %fs
# MAGIC ls dbfs:/mnt/dbacademy-users/tien-nghi.nguyen@capgemini.com/apache-spark-programming-with-databricks
