...
products_df = spark.read.csv(products_csv_path, sep = ",", header = True, inferSchema = True)
...
products_df2 = spark.read.csv(path = products_csv_path, schema= user_defined_schema, header = True, sep = ",")
...
spark.conf.set("products_csv_path", products_csv_path)
...
# MAGIC
# MAGIC val user_defined_schema = StructType(
# MAGIC   List(
# MAGIC   StructField("item_id", StringType, true),
# MAGIC   StructField("name", StringType, true),
# MAGIC   StructField("price", DoubleType, true)
# MAGIC )
# MAGIC )
# MAGIC
# MAGIC val products_csv_path = spark.conf.get("products_csv_path")
# MAGIC println(products_csv_path)
# MAGIC val products_df2 = spark.read
# MAGIC                         .option("header", "true")
# MAGIC                         .option("sep", ",")
# MAGIC                         .schema(user_defined_schema)
# MAGIC                         .csv(products_csv_path)
# MAGIC // display(products_df2.limit(5))
# MAGIC println(user_defined_schema.toDDL)
