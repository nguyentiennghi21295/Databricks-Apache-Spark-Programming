...
display(revenue_df.filter("revenue is not null"))
...
purchases_df = revenue_df.filter("ecommerce.purchase_revenue_in_usd is not null")
...
purchases_df.createOrReplaceTempView("purchases_df")
spark.sql("select event_name from purchases_df").distinct().display()
