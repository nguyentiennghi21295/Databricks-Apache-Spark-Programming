...
# MAGIC %scala
# MAGIC spark.conf.getAll.filter{ case (key, value) => key.contains("DA.paths")
# MAGIC }.foreach(println)
# MAGIC

# COMMAND ----------

# MAGIC %scala
# MAGIC println(spark.conf.get("DA.paths.users"))
# MAGIC

# COMMAND ----------

# MAGIC %fs 
# MAGIC ls dbfs:/mnt/dbacademy-users/
...
# TODO
files = dbutils.fs.ls('dbfs:/mnt/dbacademy-users/tien-nghi.nguyen@capgemini.com/')
display(files)

# COMMAND ----------

files1 = dbutils.fs.ls(DA.paths.users)
display(files1)
...
spark.conf.set("tables.users", DA.paths.users)
spark.conf.set("tables.sales", DA.paths.sales)
spark.conf.set("tables.products", DA.paths.products)
spark.conf.set("tables.events", DA.paths.events)


# COMMAND ----------

# spark.conf.get(DA.paths.users)
spark.conf.get(DA.paths.sales)
spark.conf.get(DA.paths.products)
spark.conf.get(DA.paths.events)


# COMMAND ----------

# MAGIC %sql
# MAGIC use blueberry2
...
# MAGIC %sql
# MAGIC select distinct device from Events
# MAGIC
# MAGIC limit 10;
# MAGIC -- TODO
...
# MAGIC %sql
# MAGIC select avg(purchase_revenue_in_usd) from sales limit 10;
# MAGIC -- TODO
