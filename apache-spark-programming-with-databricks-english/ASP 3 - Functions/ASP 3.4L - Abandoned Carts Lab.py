...
converted_users_df = (sales_df.select("email").dropDuplicates(["email"])
...
conversions_df = (users_df.join(converted_users_df,["email"], "left_outer")
...
abandoned_carts_df = (email_carts_df
                      .filter(col("converted") == lit(False))
