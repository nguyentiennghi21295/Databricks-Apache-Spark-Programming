...
details_df = (df
              .withColumn("items1", explode("items"))
              .select("email", "items1.item_name")
...
display(df.select("email",split(df.email, '@').getItem(0).alias('email_handle'),split(df.email, '@').getItem(1).alias('postfix_email')))
