...
# display(df)
display(df.filter(col("event_previous_timestamp").isNotNull()))

# COMMAND ----------

from pyspark.sql.functions import from_unixtime

df_converted = df.withColumn("event_previous_timestamp_converted", from_unixtime(col("event_previous_timestamp")/1000000))\
  .withColumn("event_timestamp_converted", from_unixtime(col("event_timestamp")/1000000))
...
state_purchases_df = df.groupBy("geo.state").agg(sumDistinct("ecommerce.total_item_quantity").alias("total_purchases"))
display(state_purchases_df)
...
state_aggregates_df = (df
                       .groupBy("geo.state")
                       .agg(floor(avg("ecommerce.total_item_quantity")).alias("avg_quantity"),
