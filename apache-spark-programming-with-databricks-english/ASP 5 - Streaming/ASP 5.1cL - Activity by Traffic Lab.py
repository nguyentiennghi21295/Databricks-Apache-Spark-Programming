...
from pyspark.sql.functions import col, approx_count_distinct, desc
...
static_traffic_df = spark.table("traffic_source")
pandas_df = static_traffic_df.toPandas()
pandas_df.set_index('traffic_source', inplace = True)
pandas_df = pandas_df.plot(kind='bar',figsize=(12, 7))
plt.title('Active Users by Traffic Source')
plt.xlabel('Traffic Source')
plt.ylabel('Active Users')
plt.legend().set_visible(True)  # Hide the legend if there is only one variable
plt.tight_layout()
plt.show()
